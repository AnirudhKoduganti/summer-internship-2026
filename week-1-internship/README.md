...
How to run: To run this program you need to call the methods and print out those methods to the console to see the results. For example, if I wanted to see which book has the highest word diveristy, I would print out the following:

print("Frankenstein:", calculate_word_diversity(frankText)) print("Alice:", calculate_word_diversity(aliceText)) print("Moby Dick:", calculate_word_diversity(mobyText)) print("Pride and Prejudice:", calculate_word_diversity(prideText)) print("Sherlock Holmes:", calculate_word_diversity(sherlockText))
...
3 interesting things: I found... Moby Dick has the largest unique vocabulary Alice has the highest word diversity Moby Dick and Pride and Prejudice share the most words in common